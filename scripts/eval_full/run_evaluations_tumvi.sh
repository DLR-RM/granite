...
for d in ${DATASETS[$CI_NODE_INDEX-1]}; do
   granite_vio --dataset-path  $DATASET_PATH/$d --cam-calib /usr/etc/granite/tumvi_512_eucm_calib.json \
        --dataset-type euroc --show-gui 0 --config-path /usr/etc/granite/tumvi_512_config.json \
        --result-path $folder_name/vio_$d --save-trajectory tum

   mv trajectory.txt $folder_name/${d}_granite_poses.txt

...
#include <granite/utils/apriltag.h>
...
void ApriltagDetector::detectTags(
    const granite::ManagedImage<uint16_t>& img_raw,
  ...
  detectTags(image, corners, ids, radii, corners_rejected, ids_rejected, radii_rejected);
}

void ApriltagDetector::detectTags(
    const granite::ManagedImage<uint8_t>& img_raw,
    Eigen::aligned_vector<Eigen::Vector2d>& corners, std::vector<int>& ids,
    std::vector<double>& radii,
    Eigen::aligned_vector<Eigen::Vector2d>& corners_rejected,
    std::vector<int>& ids_rejected, std::vector<double>& radii_rejected) {
  cv::Mat image(img_raw.h, img_raw.w, CV_8U);
  std::memcpy(image.ptr(), img_raw.ptr, img_raw.size() * sizeof(uint8_t));

...
#include <granite/spline/ceres_spline_helper.h>
...
void test_ceres_spline_helper_so3() {
  ...
  granite::So3Spline<N> spline(dt_ns);
  ...
  for (int64_t t_ns = 0; t_ns < spline.maxTimeNs(); t_ns += 1e8) {
...

    {
      ...
      GRANITE_ASSERT_STREAM(st_ns >= 0, "st_ns " << st_ns << " time_ns " << t_ns
                                                << " start_t_ns " << 0);

      int64_t s = st_ns / dt_ns;
      double u = double(st_ns % dt_ns) / double(dt_ns);

      GRANITE_ASSERT_STREAM(s >= 0, "s " << s);
      GRANITE_ASSERT_STREAM(size_t(s + N) <= spline.getKnots().size(),
                           "s " << s << " N " << N << " knots.size() "
      ...
      granite::CeresSplineHelper<N>::template evaluate_lie<double, Sophus::SO3>(
...
void test_ceres_spline_helper_rd() {
  ...
  granite::RdSpline<DIM, N> spline(dt_ns);
  spline.genRandomTrajectory(3 * N);
  ...
  for (int64_t t_ns = 0; t_ns < spline.maxTimeNs(); t_ns += 1e8) {
...

    {
      ...
      granite::CeresSplineHelper<N>::template evaluate<double, DIM, 0>(
          &vec[0], u, pow_inv_dt, &pos2);
      granite::CeresSplineHelper<N>::template evaluate<double, DIM, 1>(
          &vec[0], u, pow_inv_dt, &vel2);
      granite::CeresSplineHelper<N>::template evaluate<double, DIM, 2>(
  ...
  for (int i = 0; i < 2 * N; i++)
    for (double u = 0.01; u < 0.99; u += 0.01) {
...

      {
        ...
        granite::CeresSplineHelper<N>::template evaluate_lie<double,
                                                            Sophus::SE3>(
